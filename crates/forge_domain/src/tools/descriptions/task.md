...
- Optional `cwd` can be provided at the top level to set a default working directory for all tasks in the call.
- Tasks support both legacy string format and structured objects. Use `{ task: "...", cwd: "/path" }` when different tasks in one call should run with different working directories.
- When both top-level `cwd` and per-task `cwd` are provided, per-task `cwd` takes precedence for that task.
