...
///
/// All events are captured immediately via [`Client::capture`]. The SDK handles
/// its own internal batching, so we do not accumulate events ourselves.
///
...
impl Tracker {
    ...
    ///
    /// The underlying SDK client is built asynchronously on the first call to
    /// `collect` so this constructor remains synchronous.
    pub fn new(api_secret: &str) -> Self {
        Self { api_secret: api_secret.to_string(), client: OnceCell::new() }
    }

    /// Returns (or lazily initialises) the inner SDK client.
...
/// Converts an internal [`Event`] into a [`posthog_rs::Event`], mapping all
/// fields as properties and handling `$set` for person identification.
fn build_ph_event(input: &Event) -> Result<PhEvent> {
    ...
    // Set the SDK event timestamp to preserve actual event time.
